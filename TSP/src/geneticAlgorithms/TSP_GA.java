...
public class TSP_GA extends Salesman{
	 ...
	 * It then finds the length of the path performed by this chromosome and stores it into the pathDistances vector.
	 * 
	 */
	public void addToResults(IChromosome ic){
		//Create a temporary vector
		Vector<Point> temp=this.convertArrayResultToVectorPoint(ic.getGenes());
		//Add the starting point to the end of the vector so when the drawing occurs, the path is completed
	 ...
	 * 
	 * This method searches for the shortest path contained within the pathDistances vector and returns its index to the application.
	 * 
	 */
	public int getBestPathIndex(){
		...
		if(results.size()>0){
			...
			for(int i=0; i<pathDistances.size(); i++){
				//If the current value is less than the max value, a new best path has been found
