...
public class ModifiedSequentialConstructiveCrossover extends CrossoverOperator {
	...
	public ModifiedSequentialConstructiveCrossover(final Configuration a_configuration,
			final int a_desiredCrossoverRate, Vector<Point> cities) throws InvalidConfigurationException {
	...
	protected void doCrossover(IChromosome firstMate, IChromosome secondMate, List a_candidateChromosomes,
		...
		// Create the two children which are going to contain the new set of
		// chromosomes
		...
		while (parentA.size() > 0) {
			// Find the closest element to the previous element in the vector
			// child
	 ...
	 * 
	 * This method will calculate the distance between the two genes given their
	 * X and Y coordinates
	 */
	private double distance(Gene a_from, Gene a_to) {
		...
		double xValue = (citiesData.elementAt(a).getX() - citiesData.elementAt(b).getX())
				* (citiesData.elementAt(a).getX() - citiesData.elementAt(b).getX());
		double yValue = (citiesData.elementAt(a).getY() - citiesData.elementAt(b).getY())
				* (citiesData.elementAt(a).getY() - citiesData.elementAt(b).getY());
	 ...
	 * 
	 * @return Vector<Gene> Return the vector of gene created with the data from
	 * the array of Gene
	 * 
	...
	/*
	 * @param results Vector of type double containing all the distances between
	 * one point and all the other elements in the chromosome
	 * 
	 * @return int Return the index of the smallest value found within the
	 * vector
	 * 
	 * This method searches for the the smallest distance and return the index
	 * of the value
	 */
