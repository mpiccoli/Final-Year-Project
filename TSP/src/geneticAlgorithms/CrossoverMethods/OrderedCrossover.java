...
public class OrderedCrossover extends CrossoverOperator{
	...
	protected void doCrossover(IChromosome firstMate, IChromosome secondMate, List a_candidateChromosomes, RandomGenerator generator) {
		...
		//Set their value to null for an easier manipulation later on with the development of this crossover
		...
		while(!endChromosomeDone){
			...
			if(!child1.contains(secondMate.getGene(currentPosParent))){
				...
				//Increase the index value of the child once an element has been added
				currentIndexInsertionChild++;
				//Increase the index storing the number of genes inside a child
				elementsInChild++;
			}
			//Set the index of the parent 0 when the parent reaches the end of the array, increase the index otherwise
			...
			//Set the index of the child 0 when the child reaches the end of the vector
			if(currentIndexInsertionChild>secondMate.size()-1){
				currentIndexInsertionChild=0;
			}
			//If the child has got the same number of genes as the parents, it means it is completed and the loop can now end
		...
		//Convert Vector<Gene> to an array of type Gene for then passing it as a setting to the IChromosome firstMate and secondMate
